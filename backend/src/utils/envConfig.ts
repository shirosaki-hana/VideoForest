...
const envSchema = z.object({
  ...
  // === 토큰/캐시 관련 ===
  TOKEN_CACHE_TTL: z.string().optional().describe('Token cache TTL (accepts duration formats like "7d", "1w")'),
  ...
  TOKEN_CACHE: z
    ...
    .transform(val => {
      ...
      return !(lowerVal === 'off' || lowerVal === '0' || lowerVal === 'false');
    })
    .describe('Enable/disable token cache'),
});
...
function parseEnvironmentVariables() {
  try {
    const rawEnv = {
      ...
      TOKEN_CACHE: process.env.TOKEN_CACHE,
    };
